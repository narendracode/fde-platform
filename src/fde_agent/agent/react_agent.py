...
from fde_agent.config.loader import AgentConfig
from fde_agent.config.settings import settings
from fde_agent.telemetry import current_trace_id, get_tracer, jaeger_url

_log = logging.getLogger(__name__)
_tracer = get_tracer("fde_agent.agent")
...
def run_agent(
    ...
    with _tracer.start_as_current_span("agent.run") as span:
        ...
        if config.type == "supervisor":
            from fde_agent.agent.supervisor_agent import run_supervisor
